	...
	$(function(){
		$(function() {
			$.ajax({
			  ...
			  , type : "POST"
              , url : contextPath +"sample/example/getEmpList.do"
              , success : function(reponse, status, request){
            	  var data = reponse.EMP_OUT;
            	  ...
            	  for(var i = 0, len = data.length; i < len; i++){
	      			  obj = data[i];
	      			  s += '<tr>';
	      			  s += '<td name="empNo"><u>' + obj.empNo + ' </></td>';
	      			  s += '<td>' + obj.empKname + '</td>';
	      			  s += '<td>' + obj.deptCd + '</td>';
		...
		$('#p71_1 table').on('click' ,'td[name=applyId]', function(e){
    		e.preventDefault();
    		getApply($(e.target).text());
    	});

        $('#p72_1_btn1').click(function(e){
            $('#p72_1_applyId').val("");
            $('#p72_1_applyName').val("");
            $('#p72_1_phoneNum').val("");
        ...
        $('#p72_1_btn2').click(function(e){
        	document.getElementById("p62_1_Form").submit();
        });
        
        $('#p72_1_btn3').click(function(e){
        	alert("aaa");
        	WebServiceTemplete();
        });
	});
